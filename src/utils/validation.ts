...
export function sanitizeString(value: string): string {
  ...
  // HTML-escape output to neutralize any markup. No pattern-based rejection —
  // the previous dangerousPatterns check caused widespread false positives on
  // legitimate task content (e.g. "#137", "CREATE table", "curl", "ssh").
  // HTML escaping is sufficient: dangerous characters are encoded, not rejected.
  return value;
}
